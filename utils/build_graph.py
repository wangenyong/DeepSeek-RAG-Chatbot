    ...
    try:
        for idx, doc in enumerate(docs):
            ...
            try:
                ...
                logging.info("文档[%d/%d] 实体提取 | 原始文本长度：%d | 提取实体数：%d",
                             idx+1, len(docs), len(doc.page_content), len(entities))
                total_entities += len(entities)
                
                if not entities:
                    logging.info("文档[%d/%d] 未检测到有效实体", idx+1, len(docs))
                    continue

                # 记录示例实体（避免泄露敏感信息）
                sample_entities = [e[:min(5, len(e))]+"..." if len(e)>5 else e for e in entities[:3]]
                logging.info("文档[%d/%d] 示例实体：%s", idx+1, len(docs), sample_entities)
...
def retrieve_from_graph(query, G, top_k=5):
    ...
    try:
        ...
        if sorted_results:
            logging.info("TOP结果示例：%s", ["%s(%.2f)"%(x[0],x[1]) for x in sorted_results[:3]])
